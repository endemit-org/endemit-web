...
/**
 * RAVE ON VFX scene for the Vfx slice: the beat-bumping 3D logo instances on
 * top of their own CRT screen (laser zaps, TV noise, tube bloom), rendered
 * inline inside the slice section rather than as a page background.
 *
 * ⚠️ BUNDLE ISOLATION CONTRACT ⚠️
 * Only the scene files in this `theme/webgl/` directory may statically import
 * `three` and `@react-three/fiber`, and each is reachable exclusively through
 * a `next/dynamic(..., { ssr:false })` gate (this one via the Vfx slice's
 * `VfxCanvas`, `CrtScene` via `ThemeWebGLBackground`), which places three +
 * r3f + the scene in a separate lazy chunk that no page or shared chunk
 * references. Do NOT import three from anywhere else, and do NOT import this
 * file except via that dynamic gate.
 *
 * Motion runs unconditionally while visible (it IS the widget), but the
 * frameloop pauses when the tab is hidden or the slice is scrolled offscreen.
 * Motion stays within epilepsy-safe bounds: low-amplitude drift/flicker and
 * small-element beams, no large-area luminance strobe.
 */
...
function RaveonInstances({ isMobile }: { isMobile: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      // forward only that far toward it, so no side ever turns past the cap.
...
export default function RaveonVfxScene() {
  ...
  useEffect(() => {
    ...
    // Unlike the fullscreen theme background, this scene lives mid-page:
    // also pause it while the slice is scrolled out of the viewport.
  ...
  return (
    ...
    >
      {mounted && (
        <Canvas
          frameloop={hidden || offscreen ? "never" : "always"}
