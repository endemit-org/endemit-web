...
export default function AddToWalletButton({
  ...
  // Both root layouts render NextIntlClientProvider, so translations are the
  // default; explicit labels remain as an override.
  const tw = useTranslations("walletPass");
  const resolvedLabels: AddToWalletButtonLabels = labels ?? {
    addToWallet: tw("addToWallet"),
    adding: tw("adding"),
    warningTitle: tw("warningTitle"),
    warningBody: tw("warningBody"),
    cancel: tw("cancel"),
    downloadAnyway: tw("downloadAnyway"),
  };
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-neutral-900 rounded-xl p-6 max-w-sm w-full">
            <h3 className="text-lg font-semibold text-white mb-2">
              {resolvedLabels.warningTitle}
            </h3>
            <p className="text-neutral-400 mb-4">{resolvedLabels.warningBody}</p>
