...
const ARTISTS_SORTED_BY_TIME = [...ARTISTS_WITH_TIMESTAMPS].sort(
...
function ArtistCarousel({
  ...
  const isArtistLive = useCallback(
    (artist: ArtistWithTimestamp) => {
      return (
        currentTimeStamp >= artist.startTime.getTime() &&
  ...
  const scrollToRelevantArtist = useCallback(() => {
    if (!mounted || !scrollContainerRef.current || timelineItems.length === 0)
      return;
